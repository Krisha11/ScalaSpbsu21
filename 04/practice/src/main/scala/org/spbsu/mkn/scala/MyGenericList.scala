...
case object MyGenericList {
  ...
  private def insertCorrect[T](head : T, list : MyGenericList[T])(implicit comparator : Ordering[T]) : MyGenericList[T] = list match {
    case MyNil => MyGenericListClass(head, MyNil)
    case l     => {
      if (comparator.lt(head, l.head))
        MyGenericListClass(head, l)
      else
        MyGenericListClass(l.head, insertCorrect(head, l.tail))
    }
  }
